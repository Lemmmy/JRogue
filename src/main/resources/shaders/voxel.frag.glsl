...
void main() {
	...
	for (int i = 0; i < u_lights.count; i++) {
		vec3 surface_to_light = u_lights.lights[i].position - v_surfPos;
		float brightness = dot(v_normal, surface_to_light) / (length(surface_to_light) * length(v_normal));
		brightness = clamp(brightness, 0.0f, 1.0f);
		final_colour += u_lights.lights[i].colour * brightness;
